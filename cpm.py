...
class CPM:
    ...
    def get_data_from_input_file(self, cor, dur, res, max_count):
        global node_matrix
        ...
        for line1, line2, line3 in zip(fp1.readlines(), fp2.readlines(), fp3.readlines()):

            node = {}
            node['id'] = node_counter 
            node['name'] = str(node_counter)
            node['predecessor'] = line1.rstrip("\n").split(',')
            node['duration'] = line2.rstrip("\n")
            node['resource'] = line3.rstrip("\n")
            node['descendant'] = []
            node['slack'] = 0
            node['critical'] = False
            node['ES'] = 0
    ...
    def print_node_matrix(self):
        ...
        sorted_node_matrix = sorted(node_matrix, key=lambda node: node["id"])
    ...
    def find_all_activity_informations(self, file1, file2, file3, max_count):
        self.get_data_from_input_file(file1, file2, file3, max_count)
